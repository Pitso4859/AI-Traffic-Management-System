...
class LaneDetector:
    ...
    # Class-level model — loaded once, shared across instances
    ...
    def _generate_synthetic_frame() -> tuple[np.ndarray, list[tuple[int, int, int, int]]]:
        """Create a synthetic frame with random rectangles as vehicles."""
    ...
    def _annotate_frame(self, frame: np.ndarray, boxes: list, classes: list, emergency: bool) -> np.ndarray:
        ...
        cv2.putText(img, overlay_text, (8, th + 8), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

        if emergency:
            h, w = img.shape[:2]
            cv2.rectangle(img, (0, 0), (w - 1, h - 1), (0, 0, 255), 4)
            cv2.putText(img, "EMERGENCY", (w // 2 - 80, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 3)
